        ...
        <aside class="flex h-screen min-h-[700px] w-64 flex-col justify-between bg-white dark:bg-background-dark dark:border-r dark:border-slate-800 p-4 sticky top-0">
            <div class="flex flex-col gap-4">
                ...
                <nav class="flex flex-col gap-2 mt-4">
                    ...
                    <a class="flex items-center gap-3 px-3 py-2 rounded-lg bg-primary/10 text-primary" href="{{ route('products') }}">
                        <span class="material-symbols-outlined fill text-primary">inventory_2</span>
            ...
            <div class="flex flex-col p-8 gap-8">
                <div class="flex flex-wrap justify-between gap-4 items-center">
                    <div class="flex min-w-72 flex-col gap-1">
                        <p class="text-slate-900 dark:text-white text-3xl font-bold">Create Product</p>
                        <p class="text-slate-600 dark:text-slate-400 text-base">Add a new product to your inventory.</p>
                    </div>
                </div>
                    ...
                    <form method="POST" action="{{ route('products.store') }}" class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input name="sku" value="{{ old('sku') }}" class="mt-1 block w-full rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 text-sm text-slate-900 dark:text-white" />
                            </div>
                            <div>
                                <label class="text-sm font-medium text-slate-700 dark:text-slate-200">Price</label>
                                <input name="price" value="{{ old('price') }}" class="mt-1 block w-full rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 text-sm text-slate-900 dark:text-white" />
                            </div>
                        </div>

                        <div>
                            <label class="text-sm font-medium text-slate-700 dark:text-slate-200">Quantity</label>
                            <input name="quantity" value="{{ old('quantity') }}" class="mt-1 block w-full rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 text-sm text-slate-900 dark:text-white" />
                        </div>

                        <div>
                            <label class="text-sm font-medium text-slate-700 dark:text-slate-200">Description</label>
                            <textarea name="description" class="mt-1 block w-full rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 text-sm text-slate-900 dark:text-white">{{ old('description') }}</textarea>
                        </div>

                        <div class="flex items-center gap-3">
                            <button type="submit" class="bg-primary text-white px-4 py-2 rounded-lg font-semibold">Create Product</button>
