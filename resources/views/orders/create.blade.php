            ...
            <div class="flex flex-col p-8 gap-8">
                <div class="flex flex-wrap justify-between gap-4 items-center">
                    <div class="flex min-w-72 flex-col gap-1">
                        <p class="text-slate-900 dark:text-white text-3xl font-bold">Create Order</p>
                        <p class="text-slate-600 dark:text-slate-400 text-base">Fill in the order details below to create a new order.</p>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 max-w-3xl">
                    ...
                    <form method="POST" action="{{ route('orders.store') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input name="order_number" value="{{ old('order_number') }}" class="mt-1 block w-full rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 text-sm text-slate-900 dark:text-white" />
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input name="quantity" value="{{ old('quantity', 1) }}" class="mt-1 block w-full rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 text-sm text-slate-900 dark:text-white" />
                            </div>
                            <div>
                                <label class="text-sm font-medium text-slate-700 dark:text-slate-200">Total</label>
                                <input name="total" value="{{ old('total') }}" class="mt-1 block w-full rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 text-sm text-slate-900 dark:text-white" />
                            </div>
                        </div>

                        <div>
                            <label class="text-sm font-medium text-slate-700 dark:text-slate-200">Status</label>
                            <input name="status" value="{{ old('status', 'pending') }}" class="mt-1 block w-full rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 text-sm text-slate-900 dark:text-white" />
                        </div>

                        <div class="flex items-center gap-3">
                            <button type="submit" class="bg-primary text-white px-4 py-2 rounded-lg font-semibold">Create Order</button>
